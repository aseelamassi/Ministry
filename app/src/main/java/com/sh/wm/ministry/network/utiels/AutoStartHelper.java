...
public class AutoStartHelper {
    ...
    private void autoStartAsus(final Context context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            ...
            showAlert(context, (dialog, which) -> {
                try {
                    SharedPreferences.Editor loginEditor = context.getSharedPreferences("autoStart", context.MODE_PRIVATE).edit();
                    loginEditor.putBoolean("autoStart", true);
                    loginEditor.apply();
                    loginEditor.commit();
    ...
    private void showAlert(Context context, DialogInterface.OnClickListener onClickListener) {
        ...
        new AlertDialog.Builder(context).setTitle(context.getString(R.string.allow_auto_start))
